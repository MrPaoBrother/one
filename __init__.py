...
bac_images = []
bac_images.append(pygame.image.load('./resources/image/bac/background_01.png'))
bac_images.append(pygame.image.load('./resources/image/bac/background_02.png'))
...
enemy_strong_group = pygame.sprite.Group()
enemy_strong_anger_group = pygame.sprite.Group()
enemy_strong_down_group = pygame.sprite.Group()
...
while game_continue:
    ...
    bac_y -= 2
    if bac_y <= 0:
        which_bac = 1 if which_bac == 0 else 0
        bac_y = 800
    ...
    if enemy_show_frequence % 80 == 0:
        # 随机创建一个小怪图像组
        rand_enemy_imgs = enemy_imgs[randint(0, 10)]
        ...
        if enemy_show_frequence > 100:
            enemy_show_frequence = 0
    enemy_show_frequence += 1

    # 中性怪 蝙蝠等
    if enemy_strong_show_frequence % 200 == 0:
        cro = Cro(enemy_images=enemy_cro_imgs, init_pos=[randint(0, SCREEN_WIDTH - enemy_cro_imgs[0].get_rect().width), 0])
        enemy_strong_group.add(cro)
        if enemy_strong_show_frequence > 400:
            enemy_strong_show_frequence = 1
    enemy_strong_show_frequence += 1
    ...
    for enemy in enemy_group:
        if enemy.attack_frequence % 60 == 0:
        ...
        if pygame.sprite.collide_circle(enemy, player):
            ...
            enemy_down_group.add(enemy)
            enemy_group.remove(enemy)
    for strong_enemy in enemy_strong_group:
        if strong_enemy.attack_frequence % 50 == 0:
            if strong_enemy.image_index == 0:
                strong_enemy.image_index = 1
                strong_enemy.attcak(enemy_bullet_imgs)
            else:
                enemy.image_index = 0
    ...
    for mon_bullet in mon.mon_bullets:
        ...
        if pygame.sprite.collide_circle(mon_bullet, player):
            ...
            mon.mon_bullets.remove(mon_bullet)
    if not mon.final_show and randint(0, 1000) == 0:
        print "final show!!!!!"
        mon.final_show = True
    if mon.final_show:
        mon.final_move()
